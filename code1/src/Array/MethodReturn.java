package Array;
/*
题目要求：定义一个方法,求两数之和(你来计算，算完之后把结果告诉我)
题目变形：定义一个方法,打印两数之和(你来计算,算完之后你自己负责显示结果 把不用告诉我)


注意事项：
   对于有返回值的方法,可以使用单独调用,打印调用,赋值调用
   但是对于没有返回值的方法只能使用单独调用,不能使用打印调用或赋值调用.
 */
public class MethodReturn {
    public static void main(String[] args) {
        //我是main方法,我来调用你
        //我调用你，你帮我计算一下 算完之后把结果告诉我的num变量
        int num = getSum(10,20);
        System.out.println("返回值是 " + num);

        System.out.println("===========");
        printSum(10,10);

        System.out.println("===========");
        System.out.println(getSum(2,3));//正确写法
        getSum(3,5);//正确写法，但是返回值未使用

        //对于void没有返回值的方法,不能打印或者赋值
      //  System.out.println(printSum(2,3));//错误写法！
      //  System.out.println(void);//错误写法！

 //       int num2 = printSum(10,20);//错误写法！
 //       int num3 = void;

    }

    //我是一个方法 负责两数相加。
    //我有返回值int 谁调用我 我就把计算结果告诉谁
    public static int getSum(int a,int b){
        int result = a+b;
        return result;
    }

    //我是一个方法 负责两数相加
    //我没有返回值 不会把结果告诉任何人 而我自己会进行打印输出
    public static  void printSum(int a,int b){
        int result = a + b;
        System.out.println("返回值是 " + result);

    }
}
